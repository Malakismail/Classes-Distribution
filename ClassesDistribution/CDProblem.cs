...
namespace Problem
{
    ...
    public class Problem : ProblemBase, IProblem
    {
        ...
        public override string ProblemName { get { return "ClassesDistribution"; } }
        ...
        public override void TryMyCode()
        {
            ...
            //Single Lucky
            N = 10;
            int[] arr1 = { -4, -3, -1, 1, 3, 4, 6, 8, 10, 12 };
            expected = 6;
            output = ClassesDistribution.GetLuckyClass(N, arr1);
            PrintCase(N, arr1, output, expected);

            //No Lucky
            N = 10;
            int[] arr2 = { -4, -3, -1, 1, 3, 4, 5, 8, 10, 12 };
            expected = -1;
            output = ClassesDistribution.GetLuckyClass(N, arr2);
            ...
            expected = 3;
            output = ClassesDistribution.GetLuckyClass(N, arr3);
            PrintCase(N, arr3, output, expected);

            //Many Lucky
            N = 9;
            int[] arr4 = { -1, 1, 2, 3, 4, 5, 6, 7, 8};
            expected = 1;
            output = ClassesDistribution.GetLuckyClass(N, arr4);
                ...
                caseException = false;
                {
                    tstCaseThr = new Thread(() =>
                    {
                        try
                        {
                            ...
                            {
                                sum += ClassesDistribution.GetLuckyClass(N, arr);
                            }
                            output = sum / numOfRep;
                            sw.Stop();
